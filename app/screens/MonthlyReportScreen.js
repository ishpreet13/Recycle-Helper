...
import usefetchMonthlyRecycleData from "../hooks/usefetchMonthlyRecycleData";
import ListItemSeparator from "../components/ListItemSeparator";
import { BackHandler } from "react-native";

function MonthlyReportScreen({ navigation }) {
  const chook = usefetchMonthlyRecycleData();
  const userImage = chook.userImage;
  const userName = chook.userName;
  const userEmail = chook.userEmail;
  const recycleHistoryMap = chook.recycleHistoryMap;
  const totalScore = chook.totalScore;
  const totalItemsRecycled = chook.totalItemsRecycled;
  const totalMonthlyItemsRecycled = chook.totalMonthlyItemsRecycled;
  const totalMonthlyScore = chook.totalMonthlyScore;
  const itemRecycleHistoryMap = chook.itemRecycleHistoryMap;
                  ...
                  renderItem={({ item }) => (
                    <View style={styles.ruleTextContainer}>
                      ...
                      <View>
                        ...
                        >
                          {item.DailyTotalNumberOfItemsRecycled}
                        </Text>
                        <Text
                          style={{
                            fontSize: 12,
